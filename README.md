...
| File | Use |
|------|-----|
| [`LOOP_PROMPT.template.txt`](LOOP_PROMPT.template.txt) | The master loop prompt. Paste into a project worktree. |
...
# 2. scaffold a worktree + roadmap
bin/loop-new.sh <project>
# 3. fill ROADMAP.md with tight, testable rows
# 4. open the worktree, paste LOOP_PROMPT.txt, let it run
# 5. watch the board
bin/loop-status.sh
```
